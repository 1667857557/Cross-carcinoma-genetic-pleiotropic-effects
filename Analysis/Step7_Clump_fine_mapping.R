...
Aall_try <- vroom::vroom("common_factor.txt.gz") %>% mutate(P=2*pnorm(-abs(BETA/SE)), Z=BETA/SE) %>%
  inner_join(pos_idx %>% select(SNP, V5, V6, CHR, POS), by="SNP") %>%
  mutate(
    V5 = toupper(V5), V6 = toupper(V6),
    effect_allele = toupper(effect_allele),
    other_allele = toupper(other_allele)
  )

pal <- (Aall_try$effect_allele %in% c("A","T") & Aall_try$other_allele %in% c("A","T")) | 
  (Aall_try$effect_allele %in% c("C","G") & Aall_try$other_allele %in% c("C","G"))
message("Palindromic SNPs removed: ", sum(pal, na.rm=TRUE))
Aall_try <- Aall_try %>% filter(!pal)

Aall_try <- Aall_try %>% 
  mutate(
    direct = effect_allele %in% c(V5, V6),
    comp_match = comp(effect_allele) %in% c(V5, V6)
  ) %>%
  filter(direct | comp_match) %>%
  mutate(
    flip = !direct & comp_match,
    A1 = ifelse(direct, effect_allele, comp(effect_allele)),
    A2 = ifelse(A1 == V5, V6, V5),
    BETA = ifelse(flip, -BETA, BETA),
    Z = ifelse(flip, -Z, Z)
  )

message("Aligned by literal match: ", sum(Aall_try$direct, na.rm=TRUE), 
        "; aligned by complement (flipped): ", sum(Aall_try$flip, na.rm=TRUE))

Aall_try <- Aall_try %>% select(-direct, -comp_match, -flip)

abf_only <- nrow(Aall_try)==0
Aall <- if(abf_only) {
  vroom::vroom("common_factor.txt.gz") %>% 
    mutate(P=2*pnorm(-abs(BETA/SE)), Z=BETA/SE) %>%
    left_join(pos_idx %>% select(SNP, CHR, POS), by="SNP")
} else {
  Aall_try
}

n <- median(Aall$N, na.rm=TRUE)

if(!abf_only) B <- vroom::vroom("./baselineLDv2.2_annot_60.txt.gz")

sig.loci <- data.table::fread("common_factor.clumps") %>% 
  mutate(SP2=gsub("\\(\\d+\\)","",SP2), GenomicLocus=dplyr::row_number())
...
done_loci <- c(as.integer(sub("^CARMA_(\\d+)\\.rds\\.gz$","\\1",existing_carma)), 
               as.integer(sub("^ABF_(\\d+)\\.txt$","\\1",existing_abf)))
loci <- setdiff(sig.loci$GenomicLocus, done_loci); 
message("Remaining loci to process: ", paste(loci, collapse=", "))

safe_cs_idx <- function(res) tryCatch({
  cs <- res[[1]][["Credible set"]]
  if(is.null(cs)) integer(0) else unique(unlist(cs, use.names=FALSE))
}, error=function(e) integer(0))

make_psd <- function(R){
  R <- (R+t(R))/2
  ev <- eigen(R, only.values=TRUE)$values
  if(min(ev)<1e-8) diag(R) <- diag(R)+(1e-6-min(ev))
  R
}

abf_wakefield <- function(beta,se,V=0.04){
  ve <- se^2
  z <- beta/se
  sqrt(ve/(ve+V))*exp((z^2*V)/(2*(ve+V)))
}

abf_write <- function(df,locus,outdir){
  df <- df %>% 
    select(CHR, POS, Ref=other_allele, Alt=effect_allele, SNP, BETA, SE, P) %>% 
    filter(is.finite(BETA), is.finite(SE))
  
  if(nrow(df)==0) return(FALSE)
  
  df$Z <- df$BETA/df$SE
  abf <- abf_wakefield(df$BETA, df$SE, 0.04)
  pip <- abf/sum(abf)
  
  ord <- order(pip, decreasing=TRUE)
  cs <- integer(length(pip))
  
  if(length(pip)){
    k <- which(cumsum(pip[ord])>=0.95)[1]
    if(!is.na(k)) cs[ord[seq_len(k)]] <- 1L
  }
  
  out <- df %>% transmute(CHR, POS, Ref, Alt, SNP, Pval=P, Z=Z, PIP=pip, CS=cs)
  outfile <- file.path(outdir, paste0("ABF_",locus,".txt"))
  readr::write_tsv(out, outfile)
  message("ABF written: ", outfile, " (nSNPs=", nrow(df), ")")
  TRUE
}
...
for(value in loci){
  tryCatch({
    locus <- value
    row <- sig.loci %>% filter(GenomicLocus==locus) %>% slice(1)
    if(nrow(row)==0) next
    
    clump_snps <- unique(c(row$SNP[1], 
                           if(is.na(row$SP2[1])||row$SP2[1]=="") character(0) else 
                             strsplit(row$SP2[1], ",")[[1]]))
    
    span_tbl <- pos_idx %>% filter(SNP %in% clump_snps) %>% distinct(SNP, .keep_all=TRUE)
    if(nrow(span_tbl)==0 || dplyr::n_distinct(span_tbl$CHR)!=1){
      message("Skip ", locus, ": clump missing or multi-chr")
      next
    }
    ...
    # 与第一段代码一致的区域选取逻辑
    initial_start <- min(span_tbl$POS, na.rm=TRUE)
    initial_end <- max(span_tbl$POS, na.rm=TRUE)
    
    # 获取主导SNP的位置
    leading_snp <- row$SNP[1]
    leading_pos <- span_tbl$POS[span_tbl$SNP == leading_snp]
    if(length(leading_pos) == 0) {
      # 如果主导SNP不在span_tbl中，使用中间位置
      leading_pos <- (initial_start + initial_end) / 2
    }
    
    # 计算以主导SNP为中心的1Mb窗口
    window_start <- leading_pos - 500000
    window_end <- leading_pos + 500000
    
    # 确定最终区域边界
    end_cap <- max(pos_idx$POS[pos_idx$CHR==chr], na.rm=TRUE)
    
    if (window_start < initial_start | window_end > initial_end) {
      # 使用扩展窗口
      start <- max(1L, window_start)
      end <- min(end_cap, window_end)
    } else {
      # 保持原始clump区域
      start <- initial_start
      end <- initial_end
    ...
    message("Processing locus ", locus, ": chr", chr, ":", start, "-", end, 
            " (", end - start + 1, " bp)")
    
    Aloc0 <- Aall %>% filter(CHR==chr, POS>=start, POS<=end) %>% distinct(SNP, .keep_all=TRUE)
    if(nrow(Aloc0)<1){
      message("Skip ", locus, ": window empty")
      next
    }
    
    if(abf_only){
      abf_write(Aloc0, locus, position1)
      gc()
      next
    }
    ...
    repeat{
      ...
      snpfile <- tempfile(fileext=".txt")
      write.table(df2$SNP, snpfile, quote=FALSE, sep="\t", row.names=FALSE, col.names=FALSE)
      
      allelefile <- tempfile(fileext=".txt")
      write.table(df2, allelefile, quote=FALSE, sep="\t", row.names=FALSE, col.names=FALSE)
      
      pref <- file.path(position1, paste0("LD_",locus))
      st <- system2(plink_exec, c("--bfile", "/mnt/d/LD/UK10K_1KG_qc_rsid",
                                  "--extract", snpfile,
                                  "--a1-allele", allelefile,
                                  "--keep-allele-order",
                                  "--write-snplist",
                                  "--r", "square", "gz",
                                  "--out", pref))
      
      if(!file.exists(paste0(pref,".ld.gz"))){
        abf_write(Aloc0, locus, position1)
        go_next <- TRUE
        break
      }
      
      kept <- readLines(paste0(pref,".snplist"))
      mat <- as.matrix(data.table::fread(paste0(pref,".ld.gz")))
      
      if(length(kept)!=ncol(mat) || nrow(mat)!=ncol(mat)){
        abf_write(Aloc0, locus, position1)
        go_next <- TRUE
        break
      }
      ...
      # 移除NA行
      bad <- rowSums(is.na(mat))>0
      if(any(bad)){
        keep <- !bad
        mat <- mat[keep, keep, drop=FALSE]
        Aloc <- Aloc[keep, , drop=FALSE]
      }
      ...
      if(length(det)==0) break else {
        Aloc <- Aloc[-det, , drop=FALSE]
        mat <- mat[-det, -det, drop=FALSE]
        if(nrow(Aloc)==0){
          abf_write(Aloc0, locus, position1)
          go_next <- TRUE
          break
    ...
    if(is.null(annot) || nrow(annot)!=nrow(Aloc2)){
      abf_write(Aloc0, locus, position1)
      gc()
      next
    }
    ...
    annot[is.na(annot)] <- 0
    X <- as.matrix(cbind(Intercept=1, annot %>% select(-SNP)))
    
    res <- tryCatch(
      withCallingHandlers(
        CARMA(list(Aloc2$Z), list(mat), w.list=list(X), lambda.list=list(1), outlier.switch=FALSE),
        warning=function(w){invokeRestart("muffleWarning")}
      ), 
      error=function(e) e
    )
    
    if(inherits(res,"error") || is.null(res) || is.null(res[[1]]$PIPs)){
      abf_write(Aloc0, locus, position1)
      gc()
      next
    }
    
    Aloc2 <- Aloc2 %>% mutate(PIP=res[[1]]$PIPs, CS=0L)
    cs <- safe_cs_idx(res)
    if(length(cs)) Aloc2$CS[cs] <- 1L
    
    readr::write_tsv(Aloc2, file.path(position1, paste0("CARMA_",locus,".txt")))
    saveRDS(res, file.path(position1, paste0("CARMA_",locus,".rds.gz")), compress="gzip")
    gc()
    
  }, error=function(e){
    message("Skip locus ", value, ": ", conditionMessage(e))
  })
}
