...
Aall_try <- vroom::vroom("common_factor.txt.gz") %>% mutate(P=2*pnorm(-abs(BETA/SE)), Z=BETA/SE) %>%
  inner_join(pos_idx %>% select(SNP, V5, V6), by="SNP") %>%
  filter(effect_allele %in% c(V5, V6)) %>% mutate(A1=effect_allele) %>%
  filter(!(effect_allele %in% c("A","T") & other_allele %in% c("C","G")) & !(effect_allele %in% c("C","G") & other_allele %in% c("A","T")))
...
for(value in loci){
  tryCatch({
    ...
    clump_snps <- unique(c(row$SNP[1], if(is.na(row$SP2[1])||row$SP2[1]=="") character(0) else strsplit(row$SP2[1], ",")[[1]]))
